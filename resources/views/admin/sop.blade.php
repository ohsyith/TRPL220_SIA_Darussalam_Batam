...
@push('styles')
    <title>SIA Yayasan Darussalam | SOP</title> {{-- Ubah judul agar lebih generik, tidak hanya Auditor --}}
        ...
        <div class="col-lg-12">
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-body">
                    ...
                    {{-- Search (tetap ada untuk semua role yang bisa melihat SOP) --}}
                    ...
                    <div class="accordion" id="accordionSOP"
                        @if ($role == 'admin') data-sortable-url="{{ route('sop.sort') }}" @endif>
                        @foreach ($sop as $data)
                            <div class="accordion-item mb-2" data-id="{{ $data->id_sop }}">
                                <h2 class="accordion-header d-flex align-items-center gap-2 px-3"
                                    id="heading{{ $data->id_sop }}">
                                    ...
                                    @if ($role == 'admin')
                                        <button class="btn btn-sm btn-light" data-bs-toggle="modal"
                                            data-bs-target="#modalEditSop{{ $data->id_sop }}">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>
                                    @endif
                                </h2>

                                <div id="collapse{{ $data->id_sop }}" class="accordion-collapse collapse"
                                    aria-labelledby="heading{{ $data->id_sop }}" data-bs-parent="#accordionSOP">
                            ...
                            @if ($role == 'admin')
                                <div class="modal fade" id="modalEditSop{{ $data->id_sop }}" tabindex="-1"
                                    aria-labelledby="modalEditLabel{{ $data->id_sop }}" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered" style="max-width: 800px;">
                                        <div class="modal-content">
                                            <form method="POST" action="{{ route('sop.update', $data->id_sop) }}"
                                            ...
                                            <form action="{{ route('sop.destroy', $data->id_sop) }}" method="POST"
...
@push('scripts')
    ...
    <script>
        const el = document.getElementById('accordionSOP');

        // Pastikan Sortable.create hanya dijalankan jika role adalah admin
        @if ($role == 'admin')
            Sortable.create(el, {
                animation: 150,
                handle: '.drag-handle',
                onEnd: function() {
                    let order = [];
                    el.querySelectorAll('.accordion-item').forEach((item, index) => {
                        order.push({
